...
Collision Block::CheckCollisionState(sf::FloatRect playerRect)
{
	...
	if (blockRect.intersects(playerRect, intersect))
	{
		...
		{	
			if(intersect.left == playerRect.left) //겹치는 부분 왼쪽이 플레이어 왼쪽이랑 같으면 오른쪽에서 부딪힘
			{
				return Collision::Right;
			}
			else if(intersect.left >= playerRect.left)//왼쪽에서 부딪힘
			{
		...
		else //넓이가 높이보다 클 때 == 위아래서 부딪힘
		{
			if(intersect.top + intersect.height < playerRect.top + playerRect.height) 
				//겹치는 부분 바닥이 플레이어 바닥보다 작으면 아래에서 부딪힘
			{
				return Collision::Bottom;
			}
			else if(intersect.top > playerRect.top)//위에서 부딪힘
			{
